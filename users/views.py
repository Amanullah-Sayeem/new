...
from users.forms import LoginForm, AccountRegisterForm
from users.models import Account, Profile, Action, MedicalInfo
...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            login(request, user)
            request.session['alert_success'] = "Successfully registered with OTS"
            return HttpResponseRedirect('/profile/')
    else:
...
def error_denied_view(request):
    ...
    # Proceed with the rest of the view
    return render(request, 'page/error/denied.html', template_data)